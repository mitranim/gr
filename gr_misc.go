...
func UrlAppend(ref *url.URL, val interface{}) *url.URL {
	...
	/**
	The special case for non-empty hosts is supported to ensure consistency with
	equivalent URLs decoded from strings, and reversibility between
	string-encoding and string-decoding the resulting URL. When the host is not
	empty, if the path does not begin with "/", string-encoding the URL
	automatically inserts the "/" between the host and the path. The following
	URLs are distinct in structure but equivalent in text:

		&url.URL{Host: `example.com`, Path: `one`}  → `example.com/one`
		&url.URL{Host: `example.com`, Path: `/one`} → `example.com/one`

	However, decoding such a URL from text generates only one of these
	representations:

		`example.com/one` → &url.URL{Host: `example.com`, Path: `/one`}
